...
export const FavoriteCard = ({ pregunta, descripcion, fechaRespuesta, esCorresta }) => {
  ...
  return (
    <div className="max-w-sm p-4 bg-white rounded-lg border border-gray-100 shadow-md relative">
      ...
      <button 
        ...
        className="absolute top-[5px] right-[5px] focus:outline-none"
      >
        {isFavorite ? (
          <IoHeart className="text-primary w-6 h-6" /> // Icono sólido (relleno) para cuando es favorito
        ) : (
          <IoHeartOutline className="text-gray-400 w-6 h-6" /> // Icono outline (no relleno) para cuando no es favorito
        )}
      </button>
    </div>
  );
